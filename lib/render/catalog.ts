...
export const explorerCatalog = defineCatalog(schema, {
  components: {
    ...
    Map: {
      ...
      description:
        "Interactive Google Maps-style Mapbox map with a left-side overlay panel showing location cards. Renders a full-width map in the background with markers, and overlays a scrollable card panel on the left listing each marker as a selectable card. Clicking a marker or card highlights it and shows expanded details. Provide latitude and longitude for the map center. markers[] supports rich data: label (name), description (details text), address (street address), rating (0-5 number), image (thumbnail URL), category (e.g. 'Restaurant', 'Cafe'), and color (marker pin color). mapStyle: 'streets' for city, 'satellite' for terrain, 'dark' for dashboards.",
      example: {
        latitude: 37.7749,
        longitude: -122.4194,
        zoom: 12,
